...
function App() {
  ...
  const handleVerify = () => {
    if (!studentId) {
        console.error("Verification failed: Student ID is required for verification.");
  ...
  const getEvaluationStatus = () => {
    if (!isScanning && studentId && !evaluationResult) return { text: "Scanning Paused", sub: "Press Retry to scan again.", color: "yellow" };
    if (!evaluationResult) return { text: isScanning ? "SCANNING" : "Ready", sub: isScanning ? "Real-time detection active." : "System Idle.", color: isScanning ? "#60a5fa" : "white" };
    ...
    if (evaluationResult.completeness) {
      ...
      return { text: "INCOMPLETE", sub: `Missing: ${missingText}`, color: "#f87171" }; // Red
    }
  };
    ...
    <>
      ...
      <div className="top-navbar">
        <div className="nav-left">
          RVAUC-MS Compliance Detection <span style={{fontSize:'12px', opacity: 0.7}}>({wsStatus})</span>
        </div>
      </div>
